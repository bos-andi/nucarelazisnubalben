...
<div class="space-y-6">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200">
        <div class="p-6 border-b border-slate-200">
            <h1 class="text-2xl font-bold text-slate-900">Edit Program</h1>
            <p class="text-slate-600 mt-1">Perbarui informasi program Lazisnu</p>
        </div>

        <form action="{{ route('admin.programs.update', $program) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" required
                          class="w-full rounded-lg border-slate-300 focus:border-nu-500 focus:ring-nu-500"
                          placeholder="Jelaskan program ini secara singkat...">{{ old('description', $program->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
